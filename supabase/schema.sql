...
-- 3. Ensure clicks_count column exists on links table
ALTER TABLE public.links ADD COLUMN IF NOT EXISTS clicks_count INTEGER DEFAULT 0;

-- 4. Create SECURITY DEFINER RPC function to increment profile views (bypasses RLS safely for visitors)
...
-- 5. Create SECURITY DEFINER RPC function to increment link clicks (bypasses RLS safely for visitors)
...
-- 6. Grant execute permissions on RPC functions to public (anon & authenticated roles)
...
-- 7. Ensure FK from public.profiles to auth.users uses ON DELETE CASCADE
ALTER TABLE public.profiles
  DROP CONSTRAINT IF EXISTS profiles_id_fkey,
  ADD CONSTRAINT profiles_id_fkey
  FOREIGN KEY (id) REFERENCES auth.users(id) ON DELETE CASCADE;

-- 8. Ensure FK from public.links to public.profiles uses ON DELETE CASCADE
ALTER TABLE public.links
  DROP CONSTRAINT IF EXISTS links_profile_id_fkey,
  ADD CONSTRAINT links_profile_id_fkey
  FOREIGN KEY (profile_id) REFERENCES public.profiles(id) ON DELETE CASCADE;

-- 9. Ensure FK from public.user_badges to public.profiles uses ON DELETE CASCADE
ALTER TABLE public.user_badges
  DROP CONSTRAINT IF EXISTS user_badges_profile_id_fkey,
  ADD CONSTRAINT user_badges_profile_id_fkey
  FOREIGN KEY (profile_id) REFERENCES public.profiles(id) ON DELETE CASCADE;

-- 10. Trigger Function to automatically delete profile & release username when user is deleted from auth.users
...
-- 11. Trigger on auth.users table
DROP TRIGGER IF EXISTS on_auth_user_deleted ON auth.users;
CREATE TRIGGER on_auth_user_deleted
  AFTER DELETE ON auth.users
  FOR EACH ROW EXECUTE FUNCTION public.handle_user_delete();

-- 12. SECURITY DEFINER RPC function to allow authenticated users to delete THEIR OWN account from auth.users permanently
...
-- 13. SECURITY DEFINER RPC function to update user role to admin or user
